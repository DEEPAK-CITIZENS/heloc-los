...
export default function PortfolioPage() {
  ...
  const stats = useMemo(() => {
    ...
    const byState = {}
    apps.forEach(a => {
      const s = a.propertyInfo?.propertyState ?? 'Unknown'
      byState[s] = (byState[s] || 0) + 1
    })

    // Total credit line booked
    const totalBooked = apps.filter(a => a.status === 'BOOKED')
      .reduce((sum, a) => sum + (a.requestedCreditLine || 0), 0)

    return { total, approved, rate, avgCreditLine, avgScore, pipeline, scoreBuckets, byPropertyType, byState, totalBooked }
  }, [apps])
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Portfolio Analytics</h1>

      {/* KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <KpiCard label="Total Applications" value={stats.total} />
      ...
      {/* Total booked */}
      <div className="card p-5 mb-8 border-t-4 border-t-citizens-green">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Total Credit Lines Booked</p>
        <p className="text-4xl font-extrabold text-citizens-navy">{fmt(stats.totalBooked)}</p>
        <p className="text-xs text-gray-400 mt-0.5">{stats.approved} approved applications</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wide pb-2">Count</th>
                  <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wide pb-2">%</th>
                </tr>
              </thead>
              <tbody>
        ...
        {/* By state */}
        <div className="card p-5">
          <h2 className="section-label mb-4">By State / Region</h2>
          {Object.keys(stats.byState).length > 0 ? (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide pb-2">State</th>
                  <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wide pb-2">Count</th>
                  <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wide pb-2">%</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(stats.byState).sort((a, b) => b[1] - a[1]).map(([state, count]) => (
                  <tr key={state} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="py-2 font-medium text-gray-700">{state}</td>
                    <td className="py-2 text-right text-gray-800 font-semibold">{count}</td>
                    <td className="py-2 text-right text-gray-500">{stats.total > 0 ? ((count / stats.total) * 100).toFixed(1) : 0}%</td>
                  </tr>
                ))}
